...
class HAOpenStackCharm(OpenStackAPICharm):
    ...
    def configure_ssl(self, keystone_interface=None):
        ...
        with is_data_changed('charms.openstack.ssl_objects', ssl_objects) as c:
            if ssl_objects:
                if c:  # i.e. has the data actually changed
    ...
    def update_peers(self, cluster):
        ...
        for addr_type in sorted(os_ip.ADDRESS_MAP.keys()):
            ...
            hookenv.log("laddr is now: {} for {}"
                        .format(laddr, addr_type), level=hookenv.DEBUG)
            laddrs.append((addr_type, laddr))
        with is_data_changed('update_peers.laddrs', laddrs) as changed:
            if changed:
                hookenv.log("... and asking cluster to use these addresses.",
                            level=hookenv.DEBUG)
